...
function indexOfAlt(str, substr) {
  return str.toLowerCase().indexOf(substr.toLowerCase());
}
function extendHead(args) {
  document.querySelector("head").innerHTML += args;
}
function extendBody(args) {
  document.querySelector("body").innerHTML += args;
}
function googleSearch(k) {
  return `href="https://google.com/search?q=${k.trim()}" target="_blank"`;
}
...
function setRequest(args) {
  var url = location.href;
  if (url.split("?").length > 1) location.href += "&" + args;
  else location.href += "?" + args;
}
...
// DOM
function set_ddl(id, i) {
  document.getElementById(id).getElementsByTagName("option")[i].selected = true;
}
function set_txt(id, value) {
...
function toggleSearch(selector, self) {
  ...
  let classAttrib = self.className,
    addFocus = " focus";
...
function toggleAside(self) {
  var doc = document.getElementById("aside"),
    icon,
    value;
  if (!doc.style.display || doc.style.display == "none") {
    icon = "&times;";
...
function selectedLink() {
  ...
  for (var e, i = 0; i < arr.length; i++) {
    e = arr[i];
    if (e.search(".") >= 0)
      // any element with file extension
      file = e;
  }
  // console.log(file);
  ...
  for (var e, i = 0; i < a.length; i++) {
    ...
    if (e == file) a[i].setAttribute("class", "selected");
  }
}
function stickyNav(selector, top = "60px") {
  ...
  const npx = 50;
  const wst =
    document.body.scrollTop >= npx || document.documentElement.scrollTop >= npx;
  const doc = document.querySelector(selector);
  doc.style.position = "fixed";
  doc.style.zIndex = "99";
  doc.style.width = "100%";
  if (wst === true) doc.style.top = "0px";
  else doc.style.top = top;
}
...
function stickyFab(selector) {
  ...
  const wst =
    document.body.scrollTop >= npx || document.documentElement.scrollTop >= npx;
  const doc = document.querySelector(selector);
  if (wst === true) doc.style.display = "inline-block";
...
function onReset(attr) {
  ...
  for (var i = 0; i < target.length; i++) target[i].value = "";
}
...
function onChecksum(self, attr) {
  ...
  var target = document.querySelectorAll(attr);
  for (var i = 0; i < target.length; i++) target[i].checked = bool;
}
function onLogout() {
  if (confirm("Exit Application?") == true) {
...
function isDate(d) {
  var arr = d.split("-"),
    ...
    mm = arr[1],
    dd = arr[2];
  if (yyyy.length == 4 && mm.length == 2 && dd.length == 2) return true;
}
function isTime(t) {
  var arr = t.split(":"),
    hh = arr[0],
    ii = arr[1],
    ss = arr[2];
  if (hh.length == 2 && ii.length == 2 && ss.length == 2) return true;
}
